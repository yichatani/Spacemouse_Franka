...
XYZ_MIN = np.array([-2.00, -2.00, 0.05])
...
class RosPublisherThread(threading.Thread):
    def __init__(self, robot, rate=30):
        super().__init__()
        self.robot = robot
        self.rate = rate
        self.daemon = True  # 程序退出时自动结束线程
        self._running = True

    def run(self):
        rospy.init_node("franka_state_pub", anonymous=True)
        joint_pub = rospy.Publisher("/joint_states", JointState, queue_size=10)
        pose_pub  = rospy.Publisher("/franka_pose", PoseStamped, queue_size=10)
        rate = rospy.Rate(self.rate)

        msg_joint = JointState()
        msg_joint.name = [f"panda_joint{i+1}" for i in range(7)]
        msg_pose = PoseStamped()

        while not rospy.is_shutdown() and self._running:
            try:
                q = self.robot.get_joint_positions().astype(float).flatten()
                pos = self.robot.get_position().astype(float).flatten()
                quat = self.robot.get_orientation(scalar_first=False).astype(float).flatten()

                msg_joint.header.stamp = rospy.Time.now()
                msg_joint.position = q.tolist()
                joint_pub.publish(msg_joint)

                msg_pose.header.stamp = msg_joint.header.stamp
                msg_pose.header.frame_id = "panda_link0"
                msg_pose.pose.position.x, msg_pose.pose.position.y, msg_pose.pose.position.z = pos
                msg_pose.pose.orientation.x, msg_pose.pose.orientation.y, msg_pose.pose.orientation.z, msg_pose.pose.orientation.w = quat
...
def main():
    ...
    # ros_thread = RosPublisherThread(robot, rate=30)
    # ros_thread.start()
    ...
    try:
        ...
        # for _ in range(20):  # 100ms @ 200Hz，给控制器一个稳定的目标“预充”
        #     q_null = robot.q
        #     ctrl.set_control(pos0, quat0, q_nullspace=q_null)
        #     time.sleep(0.005)
        ...
        print("[INFO] Shutting down ROS thread and controller...")
        # ros_thread.stop()
        robot.stop_controller()
        time.sleep(0.5)
