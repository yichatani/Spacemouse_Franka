...
#                 grasp_state = gripper.grasp(width=0.00, speed=0.02, force=1.0, epsilon_inner=0.03, epsilon_outer=0.03)
...
#             # 把“开环时间常数”减半（停下时间常数不变）
#             tau_on_lin, tau_off_lin = 0.05, 0.03
#             tau_on_rot, tau_off_rot = 0.05, 1e-5
...
def main():
    ...
    with robot.create_context(CTRL_FREQ) as ctx, Spacemouse(max_value=500, deadzone=0.2, scale_factor=1.0) as sm:
        ...
        while ctx.ok():
            ...
            # 起步更快：把“开环时间常数”减半（停下时间常数不变）
            tau_on_lin, tau_off_lin = 0.05, 0.03   # <<< 原 0.10 → 0.05
            tau_on_rot, tau_off_rot = 0.05, 0.02   # <<< 原 0.10 → 0.05
            alpha_v = np.clip(dt / (tau_on_lin if np.any(v_cmd) else tau_off_lin), 0.0, 1.0)
            alpha_w = np.clip(dt / (tau_on_rot if np.any(w_cmd) else tau_off_rot), 0.0, 1.0)
